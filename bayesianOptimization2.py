...
from plot2 import plot_perf2, plot_acq_init, plot_acq_full, save_acq_img, load_acq_img


X = np.load("image_data_gray.npy")[:500]
...
opt.acquisition.exploration_weight=10

# Random initialization
opt.run_optimization(max_iter = 0, 
                        verbosity=True,
                        evaluations_file="eval_file_iter.txt") 
save_acq_img(opt, "acq_img_init.npy")

# Iter run
opt.run_optimization(max_iter = n_iter, verbosity=True,
                        evaluations_file="eval_file_full.txt", eps = 0.001) 

save_acq_img(opt, "acq_img_full.npy")
